...
//  Index 0: Red LED using PIC ADC, 31 ON levels
...
set0: // Control LEDR using 5-bit PIC DAC
  ...
  // Initialize DAC: DAC on, output enabled, from VDD to VSS
  DACCON0 = 0b111.0.00.0.0;

  // 5-bit DAC has 31 ON levels, we have to spread it between 1-100 % 
  // 80 = round( 31 / 100 * 256 )
  multiplier16 = reqPower;
  multiplicand8 = 80;
  mul16x8();
  // Round it before / 256
  mul16x8result += 128;
  // Make sure that light is on always when non-zero power is requested
  if ( reqPower != 0 && mul16x8result.high8 == 0 )
    mul16x8result.8 = 1;

  // Set DAC and prepare the value for the back computation of the actual power
  DACCON1 = multiplicand8 = mul16x8result.high8;

  // Convert actually used 32 level DAC value back to real 0-100 %
  // 826 = 100 / 31 * 256
  multiplier16 = 826;
  mul16x8();
  // Round it before / 256
  mul16x8result += 128;
  W = mul16x8result.high8;
  // Return powers
  goto _return
    // --------------------------------------
    ;
